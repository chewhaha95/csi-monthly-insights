...
const PACK_M = {
  ...
  changed:"July rewarded adaptation over signature. Russian logistics went dispersed and disguised the moment interdiction bit — hidden in civilian trucks, farm buildings and fuel stations — trading throughput for survivability and blinding convoy-based detection. In southern Lebanon the IDF interdicted a mobile cell on contact under persistent ISR and ran a month-long, phased clearance inside the security zone. For manoeuvre formations the message is to rebuild detection around pattern-of-life anomalies, set a minutes-latency standard for mobile targets, and disperse your own resupply before attrition forces it.",
  stats:[
  ...
  divPriorities:[
    "Rebuild logistics detection around pattern-of-life anomalies, not convoy signatures.",
    "Hold persistent ISR over any declared zone with a minutes-latency standard for mobile targets.",
  ...
  implications:[
    ...
    {ech:"Brigades", t:"Rehearse dispersed own resupply and a minutes-latency identify-to-strike cycle on mobile cells, with phased clearance under overwatch."},
  ...
  testICT:[
    ...
    "Grade a minutes-latency identify-to-strike cycle on a mobile cell in a declared zone.",
  ...
  sopReview:[
    ...
    "Mobile-target sensor-to-shooter latency standard.",
...
/* ===================== SENSE & STRIKE ===================== */
const LEARN_M = {
  "M-01":{ topic:"Dispersed, disguised logistics", status:"Action now", why:"Once interdiction bites, the adversary trades throughput for survivability and hides in civilian patterns — convoy-signature detection goes blind.", worked:"Reduced single-strike losses for Russian logistics, at the cost of slower resupply and smaller loads.", next:"Rebuild detection around pattern-of-life anomalies, and adopt dispersed own resupply before attrition forces it." },
  "M-02":{ topic:"Interdiction on contact", status:"High utility", why:"Persistent, minutes-latency ISR over a declared zone lets a mobile cell be struck on identification, and turns clearance into a sustained operation.", worked:"A motorcycle cell interdicted before contact; a month-long clearance dismantled 90+ infrastructure sites.", next:"Set a sensor-to-shooter standard for mobile targets and resource clearance as a phased, protected operation." }
};
...
const PLANNING_M = [
  {b:"Instrument the security/buffer zone", p:"For any declared zone, record the ISR coverage available against mobile small-team movement and the sensor-to-shooter latency you can actually achieve. Treat gaps in persistent coverage as the planning problem, not an afterthought."},
  {b:"Set a mobile-target engagement standard", p:"Fix a sensor-to-shooter time standard for mobile targets (e.g. under 15 minutes) and resource the ISR to meet it — persistent coverage, not periodic sweeps."},
  ...
  {b:"Rebuild logistics detection around anomalies", p:"Assume the adversary disperses and disguises supply movement under strike pressure. Re-task ISR to pattern-of-life anomalies — civilian-vehicle clustering near depots, unusual traffic at abandoned buildings — rather than convoy templates."},
...
const KEY_JUDGEMENTS_M = [
  "Under sustained strike pressure the adversary trades logistics <em>throughput</em> for survivability — convoy-signature detection goes blind.",
  "Detection doctrine must shift from convoy signatures to <em>pattern-of-life anomalies</em>.",
  "A declared zone is only controlled if ISR can strike <em>mobile</em> small teams on contact, not just fixed sites.",
  "Area clearance is a phased, protected operation — not a single-day task."
];
const SUMMARY_M = {
  "M-01":"Once under sustained ISR-strike pressure, adversary logistics disperse and disguise as civilian traffic — shift detection from convoy signatures to pattern-of-life anomalies, and adopt dispersed resupply before attrition forces it.",
  "M-02":"Persistent, low-latency ISR over a declared security zone lets a mobile cell be interdicted on contact in minutes — and turns area clearance into a phased, month-long operation, not a single-day task."
};
...
const APP_M = {
  "M-01":{ verdict:"Use it, with changes",
    lead:"Adversary logistics disperse and disguise once interdiction bites. Borrow the survivability method for our own resupply, and rebuild detection doctrine around the pattern-of-life anomalies dispersal creates.",
    div:{ d:"Assume the adversary will disperse and disguise supply movement under sustained ISR-strike pressure — plan detection around pattern-of-life anomalies, not convoy signatures.", o:"Div Comd, on G2/G4 advice.", a:"Re-tasks ISR collection to anomaly detection — civilian-vehicle clustering near known depots, unusual traffic at abandoned buildings — and updates the targeting SOP away from convoy templates.", t:"Single-strike loss rates against adversary logistics fall while resupply tempo drops — read as a shift to dispersed movement.", x:"Anomaly-based detection is collection-heavy and slower to confirm than convoy hunting.", m:"Detection method adapts within 48 hrs of the adversary's shift; targeting continues to service dispersed nodes." },
    bde:{ d:"When resupplying under contested ISR, adopt dispersed and disguised movement early — before losses force it — to preserve throughput.", o:"Bde Comd, on S4 advice.", a:"Builds a dispersed-movement resupply TTP (smaller loads, civilian-pattern routing, staggered timing) into the sustainment plan rather than reacting after attrition.", t:"Enemy ISR-strike coverage confirmed over the resupply route.", x:"Dispersed movement sacrifices throughput and speed for survivability.", m:"Resupply sustained under ISR without the single-strike losses a convoy would take." } },
  "M-02":{ verdict:"Use it, with changes",
    lead:"Interdicting a mobile cell on contact requires persistent, minutes-latency ISR over the whole zone — and area denial requires treating clearance as a phased operation, not a single day.",
    div:{ d:"Maintain persistent ISR over any declared buffer/security zone at a resolution sufficient to detect and engage mobile small-team movement — not just fixed infrastructure.", o:"Div Comd, on G2/G3 advice.", a:"Sets a sensor-to-shooter standard for mobile targets (minutes, not hours) and allocates persistent coverage rather than periodic sweeps.", t:"A declared security or buffer zone is assigned to the formation.", x:"Persistent, high-resolution coverage is ISR-intensive and hard to sustain across a wide zone.", m:"Mobile small-team movement detected and engaged inside the set latency standard." },
...
const ICT_M = {
  "M-01":{ fmt:"Route-denial exercise · Div G2/G4 + Bde S4 · 2 hrs · ISR + logistics overlay",
    ...
    dec:"Does the targeting cell switch to pattern-of-life anomaly detection in time, and does own resupply pre-empt attrition with dispersal?",
  ...
  "M-02":{ fmt:"Time-to-engage drill · Div G3 + Bde O-group · 90 min · zone ISR overlay",
    ...
    run:["Run the identify-to-strike cycle on the mobile target against a set latency standard (e.g. under 15 minutes).","Sequence a phased clearance of a nearby infrastructure cluster with standing overwatch.","Produce the documentation record for each site denied."],
...
const BLUF_TEXT_M = "In July the manoeuvre fight rewarded <b>adaptation over signature</b>. Russian logistics went dispersed and disguised the moment interdiction bit — trading throughput for survivability and blinding convoy-based detection. In southern Lebanon the IDF interdicted a <b>mobile cell on contact</b> and ran a month-long, phased clearance under persistent ISR. Shift detection to <b>pattern-of-life anomalies</b>, set a minutes-latency standard for mobile targets, and disperse your own resupply before attrition forces it.";
...
const SERIALS_S = [
  { id:"S-01", kind:"lesson", title:"Satellite-link jamming as a new EW category — Volna Kupol Garant (Ukraine)",
...
const SUMMARY_S = {
  "S-01":"A first: ground complexes jammed the Starlink satellite uplink itself — but traded concealment for power, becoming findable high-value targets that, once destroyed, restored connectivity instantly. Catalogue the signature and hunt it.",
  "S-02":"A distributed, high-volume, long-range campaign that works down a national target list to completeness degrades war-sustaining infrastructure even against a high per-raid interception rate — coverage, not cumulative damage, is the objective.",
  "S-03":"Internal-security/governance commanders are a distinct target category from military ones — their removal degrades population control, not combat capacity; watch the downstream instability as an indicator."
};
const SIGNAL_S = {
...
const APP_S = {
  ...
  "S-02":{ verdict:"Use it, with changes",
    lead:"Deny the adversary any safe node. Maintain a ranked 'last untouched target' list and resource a sustained, distributed campaign that degrades capacity even against high interception.",
    div:{ d:"Maintain a live, ranked 'last untouched target' list for any deep-strike campaign against war-sustaining infrastructure — completeness of coverage is the objective, not just cumulative damage.", o:"Div Comd, on the intelligence staff's advice.", a:"Stands up a ranked national-infrastructure target list, updated as nodes are struck, driving strike allocation toward full coverage.", t:"A deep-strike campaign against economic/war-sustaining infrastructure is authorised.", x:"Chasing completeness spreads effort thin and demands sustained munitions supply.", m:"No high-value node remains untouched; measurable degradation of adversary capacity." },
  ...
  "S-03":{ verdict:"Use it, with changes",
    ...
    div:{ d:"Maintain a parallel target package for the internal-security/governance function alongside military and financial-network targeting.", o:"Div Comd, on the intelligence staff's advice.", a:"Sorts the adversary organisation into military, financial and internal-security/governance categories, each with a distinct effects objective.", t:"An organisation enforces discipline over a population through an internal-security layer.", x:"Governance targeting invites scrutiny over effect on the civilian population and ceasefire compliance.", m:"Internal-security targets held as a distinct package with their own effects objective." },
    bde:{ d:"Watch internal-security disruption as a potential trigger for instability in an adversary-held population — collection should look for discipline breakdowns as an indicator of opportunity or of retaliation risk.", o:"Intelligence staff, on Div direction.", a:"Adds internal-order indicators to the collection plan after a governance-layer strike.", t:"An internal-security/governance commander is removed.", x:"Second-order effects are slow and hard to attribute.", m:"Discipline-breakdown indicators tracked and read for opportunity or escalation risk." } }
};
const ICT_S = {
  ...
  "S-02":{ fmt:"Deep-strike campaign design · Div intelligence + Fires + G4 · 2 hrs · target list + inventory",
    set:"A finite drone/munitions inventory and an assumed interception rate (e.g. 85%) against a national list of war-sustaining infrastructure.",
  ...
  "S-03":{ fmt:"Target categorisation exercise · Div intelligence · 90 min · adversary org chart",
    set:"An adversary organisational chart spanning military, financial and internal-security/governance functions.",
    run:["Sort targets into military, financial and internal-security/governance categories.","Assign a distinct effects objective to each category.","Identify the collection indicators that would reveal the second-order effect of an internal-security strike."],
...
const PACK_S = {
  ...
  divPriorities:[
    ...
    "Hold a parallel target package for the internal-security/governance function."
  ],
  ...
  sopReview:[
    ...
    "Parallel governance target package and internal-order indicators."
  ],
...
const PLANNING_S = [
  ...
  {b:"Keep a 'last untouched target' list", p:"For any deep-strike campaign against war-sustaining infrastructure, maintain a live ranked list and drive strike allocation toward completeness — denying the adversary any safe node is itself the objective."},
  ...
  {b:"Separate governance from military targeting", p:"Maintain a parallel target package for the internal-security/governance function, with its own effects objective, and add internal-order indicators to the collection plan to read the second-order effect of a strike."}
];
const KEY_JUDGEMENTS_S = [
  "Jamming the <em>satellite uplink</em> is a new EW category — powerful, but the emitter trades concealment for a findable signature.",
  "Destroying the jammer restores capability faster than developing a jam-resistant workaround.",
  "For a deep-strike campaign, <em>completeness of coverage</em> — denying any safe node — is the objective, not cumulative damage.",
  "Internal-security/governance commanders are a distinct target category from military ones."
];
const BLUF_S = "In July sense-and-strike turned on <b>signatures and completeness</b>. Russia jammed the <b>Starlink uplink</b> itself — a first — but the emitter was loud, and Ukraine's jammer-hunt killed it in hours, restoring connectivity instantly. Ukraine's refinery campaign worked a national list down to the <b>last untouched target</b>, driving fuel rationing across most of Russia despite high interception. And the IDF struck the <b>internal-security/governance layer</b> in Gaza as a distinct target set. Catalogue the jammer, deny every node, and package governance targets on their own.";
...
const SUMMARY_CSS = {
  "CS-01":"Tunnel demolition against confirmed military infrastructure east of an agreed line can run in parallel with ceasefire compliance — given clear standing ROE and a geolocated record that deters violation claims.",
  "CS-02":"Loss of a logistics route triggers immediate symbolic strikes on the opponent's civil-administration buildings, not a counterattack on the route — so pre-harden civil infrastructure the moment a route is recaptured.",
  "CS-03":"A siege re-strikes already-degraded infrastructure to sustain collapse, not to knock it out once — pre-position redundant, non-networked essential services for any position expected to be besieged."
};
const SIGNAL_CSS = {
...
const KEY_JUDGEMENTS_CSS = [
  "Engineer denial and ceasefire compliance can run in <em>parallel</em> — with standing ROE and a geolocated record.",
  "Loss of a route triggers a symbolic strike on <em>civil administration</em>, not a counterattack on the route.",
  "A siege re-strikes what it already broke — the objective is <em>sustaining collapse</em>, not a one-time knockout.",
  "Survivability comes from redundant, <em>non-networked</em> services pre-positioned before the siege."
];
const BLUF_CSS = "In July combat support was about <b>parallel action and redundancy</b>. IDF engineers demolished a Gaza tunnel <b>alongside</b> ceasefire compliance — proof that standing ROE plus a geolocated record lets denial and compliance run together. In Sudan, losing the El-Obeid highway drew an <b>immediate symbolic strike</b> on civil administration, not a counterattack — so pre-harden those buildings. And the 18-month siege showed infrastructure is <b>re-struck repeatedly</b>: survivability comes from redundant, non-networked services pre-positioned before the siege, and from reading the strike pattern as a campaign.";
...
const SUMMARY_EXP = {
  "EX-01":"A 'holding' ceasefire is not static: permanent construction entrenches territory diplomacy can't reverse, and zero dialogue is a higher-risk state than difficult talks — track and report both as escalatory trends.",
  "EX-02":"In a besieged or static posture the primary threat is airborne: predictable daily routines (water, resupply, markets) are drone-strike opportunities — randomise movement and build force protection around counter-UAS, not perimeter defence."
};
const SIGNAL_EXP = {
...
const KEY_JUDGEMENTS_EXP = [
  "A 'holding' ceasefire is not a static one — permanent construction can consolidate ground diplomacy cannot reverse.",
  "The <em>absence of dialogue</em> is a higher-risk indicator than difficult but active talks.",
  "For a besieged or static force the primary threat is <em>airborne</em>, not a ground incursion.",
  "The drone hunts <em>routine</em> — predictable daily movement is the target, not the perimeter."
];
const BLUF_EXP = "In July the expeditionary picture was <b>consolidation and the drone</b>. Thailand turned a 'holding' ceasefire into permanent facts on the ground — fencing and a safety perimeter while dialogue lapsed entirely, a state riskier than difficult talks. In Sudan the drone became the <b>dominant threat to static positions</b>, hunting the predictable routines of a besieged population rather than the perimeter. Report construction and the absence of dialogue as escalatory, and rebuild force protection around <b>counter-UAS and unpredictability</b>.";
...
const FRAME_M = {
  bluf:"July rewarded adaptation over signature. Russian logistics dispersed and disguised the moment Ukrainian interdiction bit, blinding convoy-based detection; in southern Lebanon the IDF interdicted a mobile cell on contact under persistent ISR and ran a month-long, phased clearance. The habits that draw fire are convoy-signature detection, single-day clearance, and reactive resupply. The response is structural: rebuild detection around pattern-of-life anomalies, set a minutes-latency standard for mobile targets, phase clearance under standing overwatch, and disperse own resupply before attrition forces it.",
  opportunities:[
    {t:"Interdict a mobile cell on contact", d:"Persistent, minutes-latency ISR over a declared zone let the IDF strike a motorcycle cell on identification — a model for controlling a zone against mobile small teams, not just fixed sites.", id:"M-02"},
    {t:"Disperse and disguise logistics under pressure", d:"Russia cut single-strike losses by hiding supply in civilian trucks, buildings and fuel stations — a survivability method worth adopting early for own resupply under contested ISR.", id:"M-01"}
  ],
  vulnerabilities:[
    {t:"Convoy-signature detection goes blind", d:"When the adversary disperses into civilian patterns, detection built on convoy signatures fails — the throughput drop is the only tell.", id:"M-01"},
    {t:"Single-day clearance and reactive resupply", d:"Treating area clearance as a one-day task, or waiting for attrition before dispersing resupply, cedes the initiative the adversary has already taken.", id:"M-02"}
  ],
  capdev:[
    {t:"Rebuild detection around anomalies", d:"Re-task ISR to pattern-of-life anomalies — civilian-vehicle clustering near depots, unusual traffic at abandoned buildings — and set a mobile-target sensor-to-shooter standard in minutes.", id:"M-01"},
    {t:"Phase clearance and pre-plan dispersed resupply", d:"Resource area clearance as a multi-day, protected operation, and build a dispersed, disguised resupply TTP into the sustainment plan from the start.", id:"M-02"}
  ]
};
const FRAME_S = {
  bluf:"July turned sense-and-strike on signatures and completeness. Russia jammed the Starlink uplink itself, but the emitter was loud and Ukraine's jammer-hunt killed it in hours; Ukraine's refinery campaign denied the adversary any untouched node, driving fuel rationing nationwide despite high interception; and the IDF struck the governance layer in Gaza as a distinct target set. The habits that waste effort are treating uplink jamming as ordinary link-jamming, measuring strike by damage rather than coverage, and lumping governance targets in with military. The response: catalogue and hunt the jammer, deny every node, and package governance targeting on its own.",
  opportunities:[
    {t:"Hunt the satellite-uplink jammer", d:"The jammer trades concealment for power, so it is findable — destroying it restored Ukrainian connectivity instantly, faster than any jam-resistant workaround.", id:"S-01"},
    {t:"Deny every node in a deep-strike campaign", d:"Working a national refinery list down to the last untouched producer degraded war-sustaining capacity nationwide even against a high per-raid interception rate.", id:"S-02"},
    {t:"Target the governance layer distinctly", d:"Striking the internal-security command layer degrades population control rather than combat power — a distinct target set with a distinct effect.", id:"S-03"}
  ],
  vulnerabilities:[
    {t:"Mis-reading uplink jamming", d:"Treating a satellite-uplink jammer as an ordinary drone-link jamming problem wastes the counter and prolongs the denial.", id:"S-01"},
    {t:"Damage-based, not coverage-based, deep strike", d:"Measuring a campaign by cumulative damage leaves the adversary safe nodes; interception then looks like success while capacity survives.", id:"S-02"},
    {t:"Undifferentiated targeting", d:"Lumping internal-security/governance commanders with military targets loses the distinct population-control effect and its second-order indicators.", id:"S-03"}
  ],
  capdev:[
    {t:"Jammer signature library + rapid strike", d:"Catalogue satellite-uplink jammer signatures as a target category and hold a jammer-hunt priority strike that restores capability on the emitter's destruction.", id:"S-01"},
    {t:"'Last untouched target' list and sustained tempo", d:"Maintain a ranked national-infrastructure list and resource sustained, distributed sortie generation sized against an assumed interception rate.", id:"S-02"},
    {t:"Parallel governance target package", d:"Hold internal-security/governance targets as a distinct package with their own effects objective and internal-order collection indicators.", id:"S-03"}
  ]
};
const FRAME_CSS = {
  bluf:"July made combat support about parallel action and redundancy. IDF engineers demolished a Gaza tunnel alongside ceasefire compliance; losing the El-Obeid highway drew an immediate symbolic strike on civil administration; and an 18-month siege re-struck already-degraded infrastructure to sustain collapse. The weaknesses are delay, mis-directed hardening and defending in place: waiting for case-by-case approval, hardening only the route, and protecting primary infrastructure instead of pre-positioning redundancy. The response: standing ceasefire-period engineer ROE with a documented record, pre-hardened retaliation targets, and redundant non-networked services set before the siege.",
  opportunities:[
    {t:"Run denial and compliance in parallel", d:"Standing ROE plus a geolocated record let engineers demolish confirmed military infrastructure east of a line without pausing for case-by-case approval or breaching the ceasefire.", id:"CS-01"},
    {t:"Predict the retaliation, not the counterattack", d:"Route loss triggered a fast symbolic strike on civil administration — pre-hardening the likely targets beats defending the route the enemy will not attack.", id:"CS-02"}
  ],
  vulnerabilities:[
    {t:"Case-by-case delay under a ceasefire", d:"Without standing ROE and an evidence standard, engineer denial stalls pending political approval and invites violation claims.", id:"CS-01"},
    {t:"Exposed civil administration and defend-in-place", d:"Unhardened banks, courts and associations within drone range are the adversary's chosen retaliation target, and defending primary infrastructure in place cedes a siege that re-strikes it.", id:"CS-03"}
  ],
  capdev:[
    {t:"Standing engineer ROE + documentation", d:"Pre-authorise the permitted ceasefire-period demolition categories with a geolocated evidence standard that deters violation claims.", id:"CS-01"},
    {t:"Pre-hardening and siege redundancy", d:"Pre-harden civil-administration retaliation targets on a recapture, and pre-position redundant, non-networked water, power and medical for any position that may be besieged.", id:"CS-03"}
  ]
};
const FRAME_EXP = {
  bluf:"July was consolidation and the drone. Thailand entrenched territory with a permanent fence and safety perimeter while dialogue lapsed entirely; in Sudan the drone became the dominant threat to static positions, hunting the predictable routines of a besieged population. The habits that invite these methods are treating a quiet ceasefire as static, deferring to official statements, and defaulting to a ground-threat posture. The response: report construction and the absence of dialogue as escalatory, assume the drone is the primary threat, and break the pattern of life around essential routines.",
  opportunities:[
    {t:"Read consolidation under a quiet ceasefire", d:"Permanent construction during a non-violent ceasefire entrenches ground diplomacy cannot reverse — an escalatory trend to detect and report, not routine calm.", id:"EX-01"},
    {t:"Treat zero dialogue as a warning", d:"A ceasefire with no bilateral contact is a higher-risk state than one with difficult talks — a distinct indicator worth reporting alongside kinetic incidents.", id:"EX-01"}
  ],
  vulnerabilities:[
    {t:"Mistaking 'holding' for static", d:"Logging only kinetic incidents lets territorial consolidation and a diplomatic breakdown pass unseen as routine quiet.", id:"EX-01"},
    {t:"Ground-threat posture against the drone", d:"For a besieged or static force the primary threat is airborne and it hunts routine — a perimeter-defence posture leaves daily movement exposed.", id:"EX-02"}
  ],
  capdev:[
    {t:"Report construction and dialogue status", d:"Add standing reporting lines for permanent construction and the state of dialogue so consolidation and diplomatic breakdown are flagged as escalatory.", id:"EX-01"},
    {t:"Counter-UAS posture and pattern-of-life discipline", d:"Re-weight force protection to counter-UAS detection and warning, and randomise essential daily movement under overhead cover.", id:"EX-02"}
  ]
};
...
const FRAME_MANEXP = {
  bluf:"July rewarded adaptation over signature — across the close fight and deployed missions alike. Logistics dispersed and disguised the moment interdiction bit, blinding convoy-based detection; a mobile cell was interdicted on contact under persistent, minutes-latency ISR; a 'holding' ceasefire concealed permanent territorial consolidation while dialogue lapsed entirely; and the drone became the dominant threat to static and besieged positions, hunting predictable routine rather than the perimeter. The response is structural: rebuild detection around pattern-of-life anomalies, set a minutes-latency standard for mobile targets, report construction and the absence of dialogue as escalatory, and rebuild static-position force protection around counter-UAS and unpredictability.",
